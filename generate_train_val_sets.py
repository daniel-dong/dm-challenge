...
if __name__ == '__main__':
    ...
    subjectIdsPos = set(metadata.subjectId[(metadata.cancerL == 1) |
                        (metadata.cancerR == 1)])
    subjectIdsNeg = subjectIds.difference(subjectIdsPos)
    subjectIdsPosTrain = set(random.sample(subjectIdsPos,
                                           int(trainSize * len(subjectIdsPos)))
                             )
    subjectIdsNegTrain = set(random.sample(subjectIdsNeg,
                                           int(trainSize * len(subjectIdsNeg)))
                             )
    print "num. training positive subjects = {}" .format(
            len(subjectIdsPosTrain)
            )
    print "num. training negative subjects = {}" .format(
            len(subjectIdsNegTrain)
            )
    ...
    metadataTrain.to_csv(os.path.join(outputDir, "exams_metadata_train.tsv"),
                         sep="\t", na_rep='.', index=False, header=True)
    metadataVal.to_csv(os.path.join(outputDir, "exams_metadata_val.tsv"),
                       sep="\t", na_rep='.', index=False, header=True)
    imagesTrain.to_csv(os.path.join(outputDir, "images_crosswalk_train.tsv"),
                       sep="\t", na_rep='.', index=False, header=True)
    imagesVal.to_csv(os.path.join(outputDir, "images_crosswalk_val.tsv"),
                     sep="\t", na_rep='.', index=False, header=True)
    print "num. training images = {}" .format(imagesTrain.shape[0])
    print "num. test images = {}" .format(imagesVal.shape[0])
